...
import wumpus2.mstt2.languageprocessing.SimpleSpeleologistSpeech;
import wumpus2.mstt2.languageprocessing.SpeleologistSpeech;
...
public class SpeleologistAgent extends Agent {
	...
	class SpeleologistBehaviour extends Behaviour {
		...
		private void discover() {
			...
			var template2 = new DFAgentDescription();
			...
			try {
				...
				var result2 = DFService.search(myAgent, template2);
		...
		public void action() {
			switch(step) {
				...
				case 2:
					...
					var state = new ACLMessage(ACLMessage.INFORM);
					state.setLanguage("English");
					state.setOntology("WumpusWorld");
					state.addReceiver(navigatorAid);
					var feelings = speech.tellPercept(currentState.percept);
				...
				case 3:
					...
					var reply2 = myAgent.receive(mt);
					if(reply2 != null) {
						// Reply received
						var action = reply2.getContent();
				...
				case 4:
					...
					var action = new ACLMessage(ACLMessage.CFP);
				...
				case 5:
					...
					var envReply = myAgent.receive(mt);
					if(envReply != null) {
						// Reply received
